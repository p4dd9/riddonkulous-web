		...
		const handleSubscribe = async () => {
			...
			try {
				...
				if (response.status === 'success') {
					setStatus('success')
					setMessage('Successfully subscribed to the weekly newsletter!')
				} else {
					setStatus('error')
	...
	return (
		<div className="w-full max-w-2xl mx-auto px-4 py-8">
			<div className="bg-[var(--color-bg)] rounded-lg shadow-lg p-8">
				<h1 className="text-2xl md:text-3xl mb-6">Newsletter Subscription</h1>
				...
				{status === 'success' && (
					<div className="space-y-4">
						...
						<p className="text-white/60">
							You will now receive our weekly newsletter with the latest riddles and updates.
						</p>
						<div className="flex gap-4">
							<Link href="/" className="flex-1">
								<BasicButton
